...
export class AboutComponent implements OnInit, AfterViewInit, OnDestroy  {
  ...
  animacion_scrollTrigger(){
    ...
    const header_texto_nombre = gsap.to(".header .texto_nombre p", { y: "-100%", duration: .5 , opacity:0, ease:'linear'});
    const header_texto_profesion = gsap.to(".header .texto_profesion p", { y: "-100%", duration: .5 , ease:'linear'});
    const textos_nombres = gsap.to(".texto_nombre", { y: "-100%", duration: .5 , position:'absolute', ease:'linear'});
    const header_img_ani1 = gsap.to(".img_header", {left:'50%', x:"-50%",top:'0%', y:"0px", duration:.2, marginTop:'100px' , ease:'linear'});
    const header_img_ani2 = gsap.to(".img_header", {position:'relative', duration: .2, height:'300px', paddingTop:'10px', width:'300px', borderRadius:'100%', background:'black', ease:'linear' });
    const header_texto_sobre_mi = gsap.to(".header .texto_sobre_mi", {opacity:1,position:'relative', duration:.2, ease:'linear'});
    const header = gsap.to(".header", {y:"-100%", duration: 1.5, ease:'linear'});
    const arrow_container_flecha = gsap.to(".arrow-container .flecha", { duration: 1, opacity:0, ease:'linear'});
    const skills_section_ani_1 = gsap.to(".skills-section", { duration: 1.5, opacity:1, ease:'linear'});
    const skills_section_ani_2 = gsap.to(".skills-section", { duration: 1.5,  y:'0%', top:'0%', ease:'linear'});
    const skills_section_ani_3 = gsap.to(".skills-section", { duration: 3, y:'-75%', top:'0%', ease:'linear'});
    const skills_section_ani_4 = gsap.to(".skills-section", { duration: 1,opacity:0, x:'-100%', ease:'linear'});
    const cuadro_hijo2_ani1 = gsap.to(".cuadro_hijo2", { duration:1, x:"-100%", ease:'linear'});
    const img_header_hijo2_ani2 = gsap.to(".img_header_hijo2", { duration:1, top:"-100%", ease:'linear'});
    const cuadro_hijo2_ani2 = gsap.to(".cuadro_hijo2", { duration: 1, y:'-40%', ease:'linear'});
    ...
    this.tl.add([header, arrow_container_flecha], .4) //header sube
    this.tl.add([skills_section_ani_1], .4)
    this.tl.add([skills_section_ani_2], .4)
    this.tl.add([skills_section_ani_3], ">")
  
    this.containerSkillBar.forEach((container)=> {
      const innerSkills = container.nativeElement.querySelectorAll('.contenedor-porcentage');
      innerSkills.forEach((innerSkill: HTMLElement, index:number) => {
        this.delay = 1.2 + index * 0.04
        const a = gsap.from(innerSkill, {width:0, duration:.3, delay:this.delay, opacity:0});
        this.tl.add(a, this.delay);
    });
  });
